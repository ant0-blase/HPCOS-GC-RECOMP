...
{
  // HPCOS fast path:
  // Chamber of Secrets is a DOL-only title. Avoid walking REL metadata for the
  // overwhelmingly common direct DOL dispatch path.
  ...
  const auto resolve_active = [&]() {
    for (u32 i = 0; i < m_active_rel_sections.size(); ++i)
    {
      ...
      if (runtime_address >= section.runtime_start &&
          static_cast<u64>(runtime_address) <
              static_cast<u64>(section.runtime_start) + section.size)
      {
        *linked_address =
  ...
  // Slow path for dynamically mapped RELs.
  RefreshRelSections();
...
{
  if (!m_module_active || !m_module || m_chunk_lookup_table.empty())
    return -1;

  /*
   * Fast path for regular DOL chunk layouts.
   *
   * DolRecomp emits the main text section as 4096-instruction chunks:
   * 4096 PPC instructions * 4 bytes = 0x4000 bytes.
   *
   * For a DOL-only title such as GHSE69 this lets us derive the chunk
   * directly from the guest PC instead of indexing the full per-instruction
   * MEM1 lookup table.
   *
   * The candidate is still validated against the module's actual range,
   * so irregular layouts fall through to the generic path.
   */
  if (m_active_rel_sections.empty())
  {
    ...
    if (num_chunks >= 3)
    {
      ...
      // Small text0 chunk preceding the regular text1 area.
      if (address < text1_base)
      {
  ...
  if (idx < 0 ||
      idx >= static_cast<int>(m_chunk_lookup_table.size()))
    return -1;
...
bool StaticRecompCore::IsFailedModuleAddress(u32 address)
{
  const int index = ChunkIndexOf(address);
  return index >= 0 && static_cast<std::size_t>(index) < m_chunk_state.size() &&
...
void StaticRecompCore::VerifyChunk(u32 index)
{
  ...
  if (hash == m_effective_chunk_hashes[index])
  {
    ...
    /*
     * Native burst is deliberately conservative:
     * - DOL/direct chunks only
     * - no REL mapping
     * - no host-call-containing chunk
     * - no forced-fallback address anywhere inside the chunk
     */
    bool chainable =
        m_active_rel_sections.empty() &&
        index < m_chunk_rel_sections.size() &&
        m_chunk_rel_sections[index] < 0 &&
    ...
    if (chainable)
    {
      for (const StaticRecompRange& range : m_forced_fallback_ranges)
      {
        if (range.start < chunk.end && range.end > chunk.start)
        {
          chainable = false;
          break;
        }
    ...
    // GHSE69 exact-PC hooks that the C++ chassis must observe. Do not let a
    // cross-chunk native burst enter their chunks invisibly:
    // - 0x801789AC: scheduler idle poll fast path
    // - 0x8000BF30: VI callback, where the game's retrace clock is virtualized
    // - 0x80038DAC: fixed-step phase, allowed once per virtual retrace
    // The chunks remain natively dispatchable; only cross-chunk chaining stops.
    ...
    if (chainable && std::string_view(m_module->game_id) == "GHSE69" &&
        ((chunk.start <= HPCOS_IDLE_PC && HPCOS_IDLE_PC < chunk.end) ||
         (chunk.start <= HPCOS_VI_CALLBACK_PC && HPCOS_VI_CALLBACK_PC < chunk.end) ||
  ...
  else
  {
    ...
    ++m_failed_chunks;
    std::fprintf(stderr,
                 "[staticrecomp] SMC: chunk [0x%08X,0x%08X) hash mismatch; interpreter "
                 "SingleStep until next invalidation (%u failed)\n",
                 chunk.start, chunk.end, m_failed_chunks);
    WARN_LOG_FMT(POWERPC,
                 "StaticRecomp: chunk [0x{:08X},0x{:08X}) failed verification (guest code "
                 "differs from module); interpreter SingleStep until next invalidation",
                 chunk.start, chunk.end);
  }
}
