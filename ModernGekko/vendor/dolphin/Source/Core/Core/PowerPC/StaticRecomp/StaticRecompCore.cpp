...
void StaticRecompCore::Init()
{
  ...
  m_native_cycle_quantum = std::clamp(
      Config::Get(Config::MAIN_STATICRECOMP_NATIVE_CYCLE_QUANTUM), 1u,
      1000000u);

  std::fprintf(stderr,
               "[staticrecomp] idle PC = %08x, native quantum = %u cycles\n",
...
void StaticRecompCore::Shutdown()
{
  ...
  std::fprintf(stderr,
               ...
               "smc_failed=%u smc_interp=%llu verifications=%llu reverify_events=%llu bursts=%llu "
               "cycles=%llu\n",
  ...
  NOTICE_LOG_FMT(POWERPC,
                 ...
                 "smc_interpreter_steps={} verifications={} reverify_events={}",
                 m_native_dispatches, m_fallback_steps, m_native_exceptions,
                 m_hook_fallback_instructions, m_failed_chunks, m_smc_interpreter_steps,
...
void StaticRecompCore::LoadModule()
{
  ...
  m_native_chain_state.assign(desc->num_chunk_ranges, 0);
