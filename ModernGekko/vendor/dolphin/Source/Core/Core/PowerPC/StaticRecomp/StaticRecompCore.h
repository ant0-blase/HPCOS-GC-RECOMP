...
private:
  ...
  bool IsFailedModuleAddress(u32 address);
  ...
  // Guest timebase is measured in TB ticks, while recomp charges
  // are CPU cycles. Keep an in-burst CPU-cycle accumulator and
  // derive TB from the live SyncIn snapshot.
  u64 m_burst_tb_base = 0;
  u64 m_burst_tb_cycles = 0;

  // D4 guard state: parallel to m_module->chunk_ranges.
  std::vector<u8> m_chunk_state;
  // ABI v4 native-burst permission map; one byte per module chunk.
  std::vector<u8> m_native_chain_state;
