...
/*
 * Local always-inline helper used by the PSQ hot path.
 * GXRuntime's public CPU header does not currently expose the
 * DolRecomp-side GXRUNTIME_ALWAYS_INLINE macro.
 */
#ifndef GXRUNTIME_ALWAYS_INLINE
...
static GXRUNTIME_ALWAYS_INLINE s32 gqr_scale(u32 value) {
...
static GXRUNTIME_ALWAYS_INLINE void psq_store_value(CPUState* cpu, u32 ea, u8 type, s32 scale, f64 value) {
    switch (type) {
    case 0:
...
static GXRUNTIME_ALWAYS_INLINE bool psq_check_enabled(CPUState* cpu, bool indexed, u32 cia) {
    ...
    u32 cia)
{
    ...
    const u8 type = (u8)((gqr >> 16) & 7u);

    /*
     * Hot GameCube PSQ float path.
     *
     * Type 0 has no quantization or scale operation.
     * Keep the exact existing PPC float conversion semantics.
     */
    if (type == 0)
    {
        cpu->fpr[frD] =
        ...
        cpu->ps1[frD] =
            w ?
                ...
                f64_value(
                    convert_to_double(
                        mem_read32(cpu, ea + 4u)));

        return true;
    }

    const s32 scale = gqr_scale(gqr >> 24);
    const u32 size = psq_type_size(type);
    ...
    cpu->fpr[frD] =
        psq_load_value(cpu, ea, type, scale);

    cpu->ps1[frD] =
        w ?
            1.0 :
            psq_load_value(
                cpu, ea + size, type, scale);
    ...
    u32 cia)
{
    ...
    const u32 gqr = cpu->gqr[gqr_index & 7u];
    const u8 type = (u8)(gqr & 7u);

    /*
     * Hot GameCube PSQ float path.
     *
     * Exact same conversion that psq_store_value(type=0)
     * previously performed, but without the generic switch.
     */
    if (type == 0)
    {
        mem_write32(
            cpu,
            ea,
            convert_to_single_ftz(
                f64_bits(cpu->fpr[frS])));
        ...
        if (!w)
        {
            mem_write32(
                ...
                ea + 4u,
                convert_to_single_ftz(
                    f64_bits(cpu->ps1[frS])));
        }

        return true;
    }

    const s32 scale = gqr_scale(gqr >> 8);
    const u32 size = psq_type_size(type);
