...
label_800031B0:
    ...
    {
        u32 ea = ctx->gpr[4] + ctx->gpr[5];
        mem_write32(ctx, ea, dolrecomp_f32_to_bits(ctx->fpr[14]));
        ctx->gpr[4] = ea;
    }
...
label_800031F4:
    ...
    ppc_fcmp(ctx, 2, ctx->fpr[3], ctx->fpr[4], false);

label_800031F8:
    ctx->pc = 0x800031F8u;
    // 800031F8: fcmpo   cr3, f5, f6
    if (!ppc_fp_available(ctx, 0x800031F8u)) return;
    ppc_fcmp(ctx, 3, ctx->fpr[5], ctx->fpr[6], true);
...
label_80004060:
    ctx->pc = 0x80004060u;
    ctx->downcount -= 2;
    // 80004060: ps_cmpo0 cr2, f1, f2
    if (!ppc_fp_available(ctx, 0x80004060u)) return;
    ppc_fcmp(ctx, 2, ctx->fpr[1], ctx->fpr[2], true);
...
label_80004068:
    ctx->pc = 0x80004068u;
    ctx->downcount -= 2;
    // 80004068: fadds.   f3, f1, f2
    if (!ppc_fp_available(ctx, 0x80004068u)) return;
    ppc_fadds(ctx, 3, 1, 2);
        ctx->cr = (ctx->cr & 0xF0FFFFFFu) | ((ctx->fpscr >> 4) & 0x0F000000u);
...
label_80004070:
    ctx->pc = 0x80004070u;
    ctx->downcount -= 2;
    // 80004070: ps_merge00  f5, f1, f2
    if (!ppc_fp_available(ctx, 0x80004070u)) return;
    {
        f64 ps0 = ctx->fpr[1];
        f64 ps1 = ctx->fpr[2];
        ctx->fpr[5] = ps0;
...
static inline DolRecompFunction dolrecomp_find_original(u32 address) {
    ...
    {
        u32 offset = address - 0x80004060u;
        if (offset < 0x00000018u && (offset & 3u) == 0u) {
            static const DolRecompFunction chunk_functions[] = {
                func_80004060,
                func_80004068,
                func_80004070,
            };
            return chunk_functions[offset / 0x00000008u];
        }
    }
    return NULL;
}
